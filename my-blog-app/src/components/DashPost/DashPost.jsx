       ...
       <div>
        ...
        <thead className="bg-gray-400">
            <tr>
              ...
              <th className="py-3 px-4 text-left font-semibold font-serif"><span >DELETE</span></th>
            </tr>
          </thead>
           <tbody>
          {userPosts.map((post) =>(
            ...
            <td className="py-3 px-6">{new Date(post.updatedAt).toLocaleDateString()}</td>
                  ...
                  <Link to = {`/update-post/${post._id}`}> {/*updation based on Id*/}
                    <button className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 cursor-pointer">Edit</button>
                    </Link>
                    </td>
                    <td>
                    <button  onClick={ ()=>{ setShowModal(true);
              setPostIdToDelete(post._id)}} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 cursor-pointer">Delete</button>
                  </td>
                </tr>
          ))}
          </tbody>
